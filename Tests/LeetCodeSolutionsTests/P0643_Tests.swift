...
final class P0643_Solution1_Tests: XCTestCase {
    ...
    func testAlternatingNumbersOddK_11() {
        ...
        XCTAssertEqual(result, expected, accuracy: 1e-5, "正负交替且k为奇数的测试用例失败 / Failed on alternating numbers with odd k test")
    }
    ...
    func testLongerArrayMiddleMax_13() {
        ...
        let k = 3
        let expected: Double = 23.33333
        let result = s.findMaxAverage(nums, k)
        print("testLongerArrayMiddleMax_13 result: \(result)")
        XCTAssertEqual(result, expected, accuracy: 1e-5, "较长数组且最大值在中间的测试用例失败 / Failed on longer array with max in the middle")
    }
    ...
    func testPerformanceWithMaxNMaxKIncreasing_19() {
        let s = P0643_Solution1()
        let nums = Array(1...100000)
        let k = 100000
        let expected = 50000.5
        self.measure {
            let result = s.findMaxAverage(nums, k)
            print("testPerformanceWithMaxNMaxKIncreasing_19 result: \(result)")
            XCTAssertEqual(result, expected, accuracy: 1e-5, "n=10^5, k=n, 递增序列的性能测试失败 / Failed on max N, max K, increasing sequence performance test")
        }
    }
    ...
    func testPerformanceWithMaxNMediumKRandom_20() {
        let s = P0643_Solution1()
        let nums = (1...100000).map { _ in Int.random(in: -10000...10000) }
        let k = 50000
        self.measure {
            // 我们无法预测确切结果，但可以确保它在范围内运行且不崩溃 / We cannot predict the exact result, but we can ensure it runs within limits and does not crash.
            let result = s.findMaxAverage(nums, k)
            print("testPerformanceWithMaxNMediumKRandom_20 result: \(result)")
            XCTAssert(result >= -10000.0 && result <= 10000.0, "n=10^5, k=5*10^4, 随机数的性能测试失败 / Result out of bounds for max N, medium K, random numbers performance test")
        }
    }
    ...
    func testPerformanceWithMaxNSmallKRandom_21() {
        ...
        self.measure {
            ...
            XCTAssert(result >= -10000.0 && result <= 10000.0, "n=10^5, k=100, 随机数的性能测试失败 / Result out of bounds for max N, small K, random numbers performance test")
        }
    }
    ...
    func testPerformanceWithMaxNAllMaxValues_22() {
        let s = P0643_Solution1()
        let nums = [Int](repeating: 10000, count: 100000)
        let k = 50000
        let expected: Double = 10000.0
        self.measure {
            let result = s.findMaxAverage(nums, k)
            print("testPerformanceWithMaxNAllMaxValues_22 result: \(result)")
            XCTAssertEqual(result, expected, accuracy: 1e-5, "n=10^5, k=n/2, 全是最大值的性能测试失败 / Failed on max N, all max values performance test")
        }
    }
}
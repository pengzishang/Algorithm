...
final class P0043_Solution1_Tests: XCTestCase {
    ...
    // Test multiplication with one digit and large number
    // 测试单数字与大数乘法
    func testSingleDigitWithLargeNumber() {
        let s = P0043_Solution1()
        let num1 = "9"
        let num2 = String(repeating: "9", count: 100)
        let result = s.multiply(num1, num2)
        print("9 * 100位9 = \(result)")
        let expected = String(repeating: "9", count: 100)
        XCTAssertEqual(result, expected, "单数字与大数乘法计算错误")
    }

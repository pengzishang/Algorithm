...
final class P1422_Solution1_Tests: XCTestCase {
    ...
    // Test where the optimal split is near the beginning.
    // 测试最佳分割点在开头附近的情况。
    func testOptimalSplit_NearStart() {
        let s = P1422_Solution1()
        let input = "10000"
        let expected = 1 // left="10", right="000" -> 1+0=1
        let result = s.maxScore(input)
        print("For input '\(input)', the max score is \(result)")
        XCTAssertEqual(result, expected, "Failed on input '10000'. Expected 1, got \(result).")
    }
    ...
    func testPerformance_Alternating() {
        ...
        let expected = 251 // left="0...0", right="1...". For "0101...01", left="0...0" (250 times), right="1". Score=250+1=251

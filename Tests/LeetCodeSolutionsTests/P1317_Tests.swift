...
final class P1317_Solution1_Tests: XCTestCase {
    ...
    func testExample1() {
        ...
        print(result)
        XCTAssertEqual(result.count, 2, "For n=\(n), the result must be an array of two integers. / 对于n=\(n)，结果必须是一个包含两个整数的数组。")
        guard result.count == 2 else { return }
        XCTAssertFalse(String(result[0]).contains("0"), "For n=\(n), the first number \(result[0]) should not contain zero. / 对于n=\(n)，第一个数字 \(result[0]) 不应包含0。")
        XCTAssertFalse(String(result[1]).contains("0"), "For n=\(n), the second number \(result[1]) should not contain zero. / 对于n=\(n)，第二个数字 \(result[1]) 不应包含0。")
        XCTAssertEqual(result[0] + result[1], n, "For n=\(n), the sum of the two numbers must be \(n). / 对于n=\(n)，两个数字的和必须等于\(n)。")
    }

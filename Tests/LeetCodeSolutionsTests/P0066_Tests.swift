...
final class P0066_Solution1_Tests: XCTestCase {
    func test1() {
    ...
    func test13() {
        ...
        XCTAssertEqual(Array(result[1...]), Array(repeating: 0, count: 100), "测试13失败: 后面100位都应该是0")
    }
...
final class P0066_Solution2_Tests: XCTestCase {
    func test1() {
        let s = P0066_Solution2()
        let digits = [1, 2, 3]
        let result = s.plusOne(digits)
        print("Test 1 - Input: \(digits), Output: \(result)")
        XCTAssertEqual(result, [1, 2, 4], "测试1失败: 123 + 1 = 124")

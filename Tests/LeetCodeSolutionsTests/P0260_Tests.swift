...
final class P0260_Solution1_Tests: XCTestCase {
    ...
    func testExample1() {
        ...
        let result = s.singleNumberIII(nums)
        print(result)
        let expected = [3, 5]
    ...
    func testNormalCaseWithPositives() {
        ...
        print(result)
        let expected = [2, 3]
        XCTAssertEqual(Set(result), Set(expected), "Normal case with positives failed: Expected [2,3] or [3,2], but got \(result)")
    }
    ...
    func testWithZerosAndPositives() {
        ...
        print(result)
        let expected = [1, 2]
        XCTAssertEqual(Set(result), Set(expected), "Case with zeros and positives failed: Expected [1,2] or [2,1], but got \(result)")
    }
    ...
    func testWithNegativesAndPositives() {
        ...
        print(result)
        let expected = [-2, 3]
        XCTAssertEqual(Set(result), Set(expected), "Case with negatives and positives failed: Expected [-2,3] or [3,-2], but got \(result)")
    }
    ...
    func testBoundaryValues() {
        ...
        let result = s.singleNumberIII(nums.map { Int($0) })
        print(result)
        let expected = [Int(Int32.max), 0]
        XCTAssertEqual(Set(result), Set(expected), "Boundary values failed: Expected [Int32.max, 0] or [0, Int32.max], but got \(result)")
    }
    ...
    func testMinLengthArray() {
        ...
        print(result)
        let expected = [5, 6]
        XCTAssertEqual(Set(result), Set(expected), "Min length array failed: Expected [5,6] or [6,5], but got \(result)")
    }
    ...
    func testAnotherNormalCase() {
        ...
        print(result)
        let expected = [3, 4]
        XCTAssertEqual(Set(result), Set(expected), "Another normal case failed: Expected [3,4] or [4,3], but got \(result)")
    }
    ...
    func testLargeArrayPerformance() {
        ...
        XCTAssertEqual(Set(result), Set(expected), "Large array performance test failed: Expected [15000,15001] or [15001,15000], but got \(result)")
    }
    ...
    func testUniqueInMiddle() {
        ...
        print(result)
        let expected = [2, 5]
        XCTAssertEqual(Set(result), Set(expected), "Unique in middle failed: Expected [2,5] or [5,2], but got \(result)")
    }

    // Case with negative zeros and positives / 包含负零和正数的情况
    func testWithNegativeZeros() {
        let s = P0260_Solution1()
        let nums = [0, -0, 1, 2] // In Swift, 0 and -0 are equal, so this might not be distinct, but for test we use values / 在Swift中，0和-0相等，因此可能不区分，但测试中我们使用值
        // Actually, in Swift, 0 and -0 are both 0, so this array has duplicates. Let's use distinct negatives / 实际上，在Swift中0和-0都是0，所以这个数组有重复。我们使用不同的负数。
        let adjustedNums = [-1, 0, 1, -1] // Change to avoid issue / 修改以避免问题
        let result = s.singleNumberIII(adjustedNums)
        print(result)
        let expected = [0, 1]
        XCTAssertEqual(Set(result), Set(expected), "With negative zeros failed: Expected [0,1] or [1,0], but got \(result)")
    }
}